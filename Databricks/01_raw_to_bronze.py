...
movie_raw = spark.read.json(path = f"/FileStore/movies", multiLine = True)
...
movie_raw = (movie_raw.select("movie",
lit("files.data.databricks.com").alias("datasource"),
...
# TODO
spark.sql("""
DROP TABLE IF EXISTS movie_raw_bronze;
""")

spark.sql(f"""
CREATE TABLE movie_raw_bronze
USING DELTA
LOCATION "{bronzePath}"
""")
...
# MAGIC %sql
# MAGIC 
# MAGIC SELECT * FROM movie_raw_bronze
